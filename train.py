...
from utils.metrics_plots import  calculate_metrics, calculate_metrics_all
...
import cv2
from torchvision.utils import save_image
from dataset import h5_dataset, split_list_ratio
...
def build_network(checkpoint, opt):
    ...
    model = FrenetPointNetCls(k=opt.num_classes, use_frenet=False) 
    model.to(opt.device)
...
def iter_one_epoch(dataLoader: DataLoader,
    ...
    train_labels_lst, train_predicted_lst, train_prob_lst = [], [], []
    ...
    for step, datas in enumerate(bar):
        ...
        points = points.transpose(2, 1)  # points [B, 12, N]
        ...
        train_prob_lst.extend(probs.detach().cpu().numpy())
    ...
    mac_precision, mac_recall, mac_f1, accuracy, auc = calculate_metrics_all(
        train_labels_lst,
        train_predicted_lst,
        train_prob_lst
    )
    ...
    return model, loss_mean, mac_precision, mac_recall, mac_f1, accuracy, auc
...
def train(opt):
    ...
    for epoch in range(start_epoch, start_epoch + opt.epochs):
        # train one epoch
        model, loss_mean, mac_precision, mac_recall, mac_f1, accuracy, auc = iter_one_epoch(
            dataLoader=train_dl,
            model=model,
            optimizer=optimizer,
        ...
        writer.add_scalar(f'Training precision', mac_precision, epoch)
        ...
        writer.add_scalar(f'Training auc', auc, epoch)
        ...
        if (epoch + 1) % opt.val_freq == 0:
            with torch.no_grad():
                ...
                writer.add_scalar(f'Val precision', mac_precision, epoch)
                ...
                writer.add_scalar(f'Val auc', auc, epoch)
